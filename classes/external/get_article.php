...
class get_article extends external_api {
    ...
    public static function execute(int $articleid): array {
        ...
        if (empty($config->enabled) || $apikey === '' || $portalurl === '') {
            debugging('local_freshdesk: article fetch skipped — plugin disabled or portal URL / API key not configured.',
                DEBUG_DEVELOPER);
            return $empty;
        }
        ...
        if ($httpcode !== 200) {
            debugging('local_freshdesk: article fetch failed (HTTP ' . $httpcode . '): ' .
